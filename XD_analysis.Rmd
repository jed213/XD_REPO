...
sc = scale_color_manual(values = c("black", "lightpink", "red"))
```
...
```
I am happy with how this looks, the couple samples with higher % of reads removed had low read counts anyway. Last, I need to remove the frogs that were allowed to metamorphose.

```{r phyloseq cleanup, include = FALSE}
# remove samples that were allowed to metamorphose
XD.clean <- subset_samples(XD.decontam, age.days <= 100)
```
...
## (TO-DO) Statistics: Is thyroxine stunting growth?

To test this, I could construct a model where mass is the response variable, and treatment/exposure and age are the predictor variables. This could tell me whether treatment/exposure has an effect on growth while controlling for the increase in age over time.
...
model <- lm(shannon ~ tank.id + dna.ng.ul.LOG + mass.g.LOG + treatment + 
              body.condition + exposureYN + shannon + age.days.S + stage.S, 
...
car::Anova(model5)
```
DNA concentration was by far the biggest predictor of shannon diversity in these tadpoles, which limits our interpretation of what may be happening biologically. Both age and stage were also significant predictors, with stage explaining slightly more variation in shannon diversity.

Trying more versions of the model with and without DNA included.  
With DNA:  
9.1. Shannon ~ Age + Stage + DNA + Mass  
9.2. Shannon ~ Age + Stage + DNA + Treatment  
9.3. Shannon ~ Age + Stage + DNA + Tank ID  
...
```{r new models}
model9.1 <- lm(shannon ~ age.days.S + stage.S + dna.ng.ul.LOG, + mass.g.LOG, 
             data = alpha.pairs.scaled)
model9.2 <- lm(shannon ~ age.days.S + stage.S + dna.ng.ul.LOG + treatment, 
             data = alpha.pairs.scaled)
model9.3 <- lm(shannon ~ age.days.S + stage.S + dna.ng.ul.LOG + tank.id, 
             data = alpha.pairs.scaled)
...
AIC(model10.5)
```
Overall, the models with DNA included are a better fit, as expected by the previous results. When DNA is not included, models 10.5, 1, 4, 6 and 6 are the next best fits. Two of these are just age and stage alone (with the best one having interaction term), and the other two are age/stage/mass and age/stage/bodycondition. I am going to dig deeper into the age/stage + interaction alone model, to see the relative weights of each variable in that model.  

```{r model10.5 validation}
# Residuals vs fitted values
plot(residuals(model10.5, type="response") ~ model5$fitted.values)
abline(0,0,col="gray")

# Residuals vs Age
plot(residuals(model10.5, type="response") ~ model5$model$age.days.S)
abline(0,0,col="gray")

# Residuals vs Stage
plot(residuals(model10.5, type="response") ~ model5$model$stage.S)
abline(0,0,col="gray")
```
...
car::Anova(model10.5)
```
Interestingly, age is appearing as super significant while stage and even the interaction term are borderline non-significant. This aligns with the rest of the data that tends to show age having a greater effect than stage.
...
Everything is very significant, but the F-stat for age is WAY higher than any of the others, including the stat for stage. In fact, stage may have had less of an effect than treatment.
...
```{r ANCOMBC exposure, warnings = FALSE, message = FALSE, echo = FALSE}
set.seed(14)
...
```{r ANCOMBC treatment pairwise, warnings = FALSE, message = FALSE, echo = FALSE}
set.seed(14)
