...
To find the index I will first make the log variables, then run the model and save the residuals as a body condition variable.  
...
Now I want to see whether the three groups (Control, Thyroxine, Delayed Thyroxine) have any taxa that are differentially abundant between them. I start by comparing Control vs (Thyroxine + Delayed Thyroxine) to see whether receiving thyroxine in general altered the microbiome, then will do a pairwise comparison and then split the samples into Control vs Thyroxine vs Delayed Thyroxine.